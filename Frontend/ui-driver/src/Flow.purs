...
benefitsScreenFlow = do
  ...
  case benefitsScreen of
    ...
    GO_TO_KEYSTONE_STOREFRONT _ -> keystoneStorefrontFlow

keystoneStorefrontFlow :: FlowBT String Unit
keystoneStorefrontFlow = do
  output <- UI.keystoneStorefrontScreen
  case output of
    GO_TO_KEYSTONE_PRODUCT_DETAIL product _ -> do
      modifyScreenState $ KeystoneProductDetailScreenStateType (\s -> s { data = s.data { slug = product.slug, product = Just (keystoneProductToDetail product), error = Nothing }, props = s.props { isLoading = false, quantity = 1 } })
      keystoneProductDetailFlow
    GO_TO_KEYSTONE_CART_FROM_STOREFRONT state -> do
      modifyScreenState $ KeystoneCartScreenStateType (\s -> s { data = s.data { cart = state.data.cart } })
      keystoneCartFlow

keystoneProductToDetail :: ST.KeystoneProduct -> ST.KeystoneProductDetail
keystoneProductToDetail product =
  { id: product.id
  , name: product.name
  , slug: product.slug
  , brand: product.brand
  ...
  , images: if DA.null product.images then (if product.image == "" then [] else [ product.image ]) else product.images
  , features: product.features
  }

keystoneProductDetailFlow :: FlowBT String Unit
keystoneProductDetailFlow = do
  output <- UI.keystoneProductDetailScreen
  case output of
    GO_TO_KEYSTONE_CART_FROM_DETAIL state -> do
      modifyScreenState $ KeystoneCartScreenStateType (\s -> s { data = s.data { cart = state.data.cart } })
      keystoneCartFlow
    GO_TO_KEYSTONE_STOREFRONT_FROM_DETAIL tab -> do
      modifyScreenState $ KeystoneStorefrontScreenStateType (\s -> s { props = s.props { activeTab = tab } })
      keystoneStorefrontFlow

keystoneCartFlow :: FlowBT String Unit
keystoneCartFlow = do
  output <- UI.keystoneCartScreen
  case output of
    GO_TO_KEYSTONE_STOREFRONT_FROM_CART tab -> do
      modifyScreenState $ KeystoneStorefrontScreenStateType (\s -> s { props = s.props { activeTab = tab } })
      keystoneStorefrontFlow
